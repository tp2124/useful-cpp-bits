...
void PoolAllocator<block_size, num_blocks>::StartUp()
{
	...
	{
		m_pPool[i] = PoolBlock<block_size>();

#ifdef _DEBUG
			m_pPool[i]._memory[0] = 0xDE;
...
{
	Dbg_Assert(size <= block_size, "allocation request");
	Dbg_Assert(iBlocksFree > 0, "no blocks free :(")

	PoolBlock<block_size>* temp;
	if(iBlocksFree)
	{	
		--iBlocksFree;
		temp = m_pFreeList;
...
void PoolAllocator<block_size, num_blocks>::Free(void* ptr)
{
	...
	PoolBlock<block_size>* temp = reinterpret_cast<PoolBlock<block_size>* >(ptr);
#ifdef _DEBUG 
		Dbg_Assert(temp->_boundary != 0xDEADBEEF, "bounds were overwritten");
		temp->_memory[0] = 0xDE;
#endif
	temp->_next = m_pFreeList;
	m_pFreeList = temp;
	++iBlocksFree;
}
